...
public:
  ...
  //bool                m_ce {false};   //!< DCTCP.CE (receiver) => this should be simulated by m_ecnState == TcpSocketState::ECN_CE_RCVD
  double              m_alpha {0};    //!< DCTCP.Alpha (sender)
  SequenceNumber32    m_windowEnd {0};//!< DCTCP.WindowEnd (sender)
  uint32_t            m_bytesAcked {0};     //!< DCTCP.BytesAcked (sender)
  uint32_t            m_bytesMarked {0};    //!< DCTCP.BytesMarked (sender)
  Ptr<TcpRxBuffer>    m_rxBuffer;     //!< Rx buffer: migrated from tcp-socket-base
